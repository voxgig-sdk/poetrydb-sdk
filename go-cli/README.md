...
boru-driven command-line client **and** interactive REPL for the Poetrydb
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/poetrydb-cli)
make build
...
./poetrydb-cli list author
./poetrydb-cli load 1 author            # {id:1} shorthand
./poetrydb-cli load '{id:1}' author       # explicit match map
./poetrydb-cli list authorab

# 5. Override the API base URL for a single call
POETRYDB_BASE=https://api.example.com ./poetrydb-cli list author

# 6. No arguments -> interactive REPL
./poetrydb-cli
poetrydb> list author
poetrydb> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/poetrydb-cli
   ```
...
```sh
./poetrydb-cli load 1 author          # scalar shorthand for {id:1}
./poetrydb-cli load '{id:1}' author     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `poetrydb>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/poetrydb-cli`. |
...
(`./poetrydb-cli <expr>`) and interactively (the REPL), and expressions compose the
