// Command Poetrydb-cli is a CLI + REPL for the Poetrydb SDK,
...
const prompt = "poetrydb"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "author authorab combined_search combined_search_with_field line linecount poemcount random title titleab"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      POETRYDB_APIKEY (api key), POETRYDB_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: POETRYDB_APIKEY carries the API key and
	// POETRYDB_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("POETRYDB_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("POETRYDB_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewPoetrydbSDK(opts)
