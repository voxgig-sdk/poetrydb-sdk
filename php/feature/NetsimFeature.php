...
class PoetrydbNetsimFeature extends PoetrydbBaseFeature
{
    ...
    private function _simulate(PoetrydbContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(PoetrydbContext $ctx, array $applied): void
    {
        $client = $this->client;
