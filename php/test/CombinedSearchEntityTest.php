...
class CombinedSearchEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = PoetrydbSDK::test(null, null);
        $ent = $testsdk->CombinedSearch(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->CombinedSearch(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->CombinedSearch(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = combined_search_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "combined_search." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set POETRYDB_TEST_COMBINED_SEARCH_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $combined_search_ref01_data = null;
        if (count($combined_search_ref01_data_raw) > 0) {
            $combined_search_ref01_data = Helpers::to_map($combined_search_ref01_data_raw[0][1]);
        }

        // LIST
        $combined_search_ref01_ent = $client->CombinedSearch(null);
...
function combined_search_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/combined_search/CombinedSearchTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["combined_search01", "combined_search02", "combined_search03", "input_field101", "input_field201", "search_term101", "search_term201"] as $k) {
    ...
    $entid_env_raw = getenv("POETRYDB_TEST_COMBINED_SEARCH_ENTID");
