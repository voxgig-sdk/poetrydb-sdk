...
class TitleEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = PoetrydbSDK::test(null, null);
        $ent = $testsdk->Title(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Title(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Title(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = title_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "title." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set POETRYDB_TEST_TITLE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $title_ref01_data = null;
        if (count($title_ref01_data_raw) > 0) {
            $title_ref01_data = Helpers::to_map($title_ref01_data_raw[0][1]);
        }

        // LIST
        $title_ref01_ent = $client->Title(null);
        $title_ref01_match = [];

        $title_ref01_list_result = $title_ref01_ent->list($title_ref01_match, null);
...
function title_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/title/TitleTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["title01", "title02", "title03"] as $k) {
    ...
    $entid_env_raw = getenv("POETRYDB_TEST_TITLE_ENTID");
