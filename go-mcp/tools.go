...
type Args struct {
	Entity string         `json:"entity" jsonschema:"author | authorab | combined_search | combined_search_with_field | line | linecount | poemcount | random | title | titleab"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.PoetrydbSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "poetrydb_list",
		Description: "List records from Poetrydb. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "poetrydb_load",
		Description: "Load a single record from Poetrydb. " +
...
func runOp(client *sdk.PoetrydbSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.PoetrydbSDK, name string) (sdk.PoetrydbEntity, error) {
	switch strings.ToLower(name) {
	case "author":
		return client.Author(nil), nil
	case "authorab":
		return client.Authorab(nil), nil
	case "combined_search":
		return client.CombinedSearch(nil), nil
	case "combined_search_with_field":
		return client.CombinedSearchWithField(nil), nil
	case "line":
		return client.Line(nil), nil
	case "linecount":
		return client.Linecount(nil), nil
	case "poemcount":
		return client.Poemcount(nil), nil
	case "random":
		return client.Random(nil), nil
	case "title":
		return client.Title(nil), nil
	case "titleab":
		return client.Titleab(nil), nil
