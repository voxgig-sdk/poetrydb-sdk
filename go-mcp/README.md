...
[MCP](https://modelcontextprotocol.io) server exposing the Poetrydb SDK as
two agent tools — `poetrydb_list` and `poetrydb_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/poetrydb-mcp)
make build

# 2. Provide credentials via the environment
export POETRYDB_APIKEY=sk_live_xxx
...
```jsonc
// poetrydb_list: first page of records
{ "entity": "author" }
{ "entity": "author", "query": { } }

// poetrydb_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/poetrydb-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user poetrydb \
     -- "$PWD"/dist/*/poetrydb-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `poetrydb_list` and `poetrydb_load` tools now appear
   in new sessions. Ask the agent to *"list author using poetrydb"*
   and it calls `poetrydb_list` with `{"entity":"author"}`.
...
```sh
export POETRYDB_APIKEY=sk_live_xxx            # API key
export POETRYDB_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `poetrydb_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `poetrydb_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `POETRYDB_APIKEY` | API key sent with every request. |
| `POETRYDB_BASE` | Optional override of the API base URL. |
