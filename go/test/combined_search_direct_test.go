...
func TestCombinedSearchDirect(t *testing.T) {
	t.Run("direct-list-combined_search", func(t *testing.T) {
		setup := combined_searchDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-combined_search", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"input_field101", "input_field201", "search_term101", "search_term201"} {
		...
		if setup.live {
			...
			params["search_term1"] = "direct03"
		}
		if setup.live {
			params["search_term2"] = setup.idmap["search_term201"]
		} else {
			params["search_term2"] = "direct04"
		}
...
type combined_searchDirectSetupResult struct {
...
func combined_searchDirectSetup(mockres any) *combined_searchDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["POETRYDB_TEST_COMBINED_SEARCH_ENTID"]; ok {
		...
		return &combined_searchDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &combined_searchDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
