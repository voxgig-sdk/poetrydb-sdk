...
import (
	...
	omni "github.com/voxgig-sdk/poetrydb-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.PoetrydbError); is {
				out := map[string]any{
					"name":    "PoetrydbError",
...
func omniCtx(arg any, client *sdk.PoetrydbSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.PoetrydbSDK); is {
			client = live
