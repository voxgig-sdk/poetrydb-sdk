...
func TestCombinedSearchWithFieldEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.CombinedSearchWithField(nil)
		if ent == nil {
			t.Fatal("expected non-nil CombinedSearchWithFieldEntity")
		}
	})
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.CombinedSearchWithField(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.CombinedSearchWithField(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		setup := combined_search_with_fieldBasicSetup(nil)
		...
		for _, _op := range []string{"list"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "combined_search_with_field." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		_ = combinedSearchWithFieldRef01Data

		// LIST
		combinedSearchWithFieldRef01Ent := client.CombinedSearchWithField(nil)
		combinedSearchWithFieldRef01Match := map[string]any{
		...
		combinedSearchWithFieldRef01ListResult, err := combinedSearchWithFieldRef01Ent.List(combinedSearchWithFieldRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, combinedSearchWithFieldRef01ListOk := combinedSearchWithFieldRef01ListResult.([]any)
		if !combinedSearchWithFieldRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", combinedSearchWithFieldRef01ListResult)
		}

	})
}

func combined_search_with_fieldBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "combined_search_with_field", "CombinedSearchWithFieldTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read combined_search_with_field test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse combined_search_with_field test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("POETRYDB_TEST_COMBINED_SEARCH_WITH_FIELD_ENTID")
