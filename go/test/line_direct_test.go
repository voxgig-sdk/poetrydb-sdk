...
func TestLineDirect(t *testing.T) {
	t.Run("direct-list-line", func(t *testing.T) {
		setup := lineDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-line", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"format01", "line01", "output_field01"} {
	...
	t.Run("direct-load-line", func(t *testing.T) {
		setup := lineDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-line", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type lineDirectSetupResult struct {
...
func lineDirectSetup(mockres any) *lineDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"POETRYDB_TEST_LINE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["POETRYDB_TEST_LINE_ENTID"]; ok {
		...
		return &lineDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &lineDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
