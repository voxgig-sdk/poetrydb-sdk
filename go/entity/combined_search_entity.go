...
type CombinedSearchEntity struct {
	name    string
...
func NewCombinedSearchEntity(client *core.PoetrydbSDK, entopts map[string]any) *CombinedSearchEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *CombinedSearchEntity) GetName() string { return e.name }
...
func (e *CombinedSearchEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *CombinedSearchEntity) Deleted() bool {
	return e.deleted
}


func (e *CombinedSearchEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewCombinedSearchEntity(e.client, opts)
}

func (e *CombinedSearchEntity) Data(args ...any) any {
...
func (e *CombinedSearchEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an CombinedSearch; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *CombinedSearchEntity) DataTyped(data ...CombinedSearch) CombinedSearch {
	if len(data) > 0 {
		return typedFrom[CombinedSearch](e.Data(asMap(data[0])))
	}
	return typedFrom[CombinedSearch](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through CombinedSearch (all fields
// optional at the wire level).
func (e *CombinedSearchEntity) MatchTyped(match ...CombinedSearch) CombinedSearch {
	if len(match) > 0 {
		return typedFrom[CombinedSearch](e.Match(asMap(match[0])))
	}
	return typedFrom[CombinedSearch](e.Match())
}
...
func (e *CombinedSearchEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *CombinedSearchEntity) Load(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("load", e.name)
}



func (e *CombinedSearchEntity) List(reqmatch map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// CombinedSearchListMatch and returns []CombinedSearch. It delegates to the untyped
// List (identical runtime) and converts at the typed boundary.
func (e *CombinedSearchEntity) ListTyped(reqmatch CombinedSearchListMatch, ctrl map[string]any) ([]CombinedSearch, error) {
	res, err := e.List(asMap(reqmatch), ctrl)
	if err != nil {
		return nil, err
	}
	return typedSliceFrom[CombinedSearch](res), nil
}



func (e *CombinedSearchEntity) Create(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("create", e.name)
}


func (e *CombinedSearchEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *CombinedSearchEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *CombinedSearchEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
