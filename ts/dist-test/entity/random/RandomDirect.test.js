...
(0, node_test_1.describe)('RandomDirect', async () => {
    ...
    (0, node_test_1.test)('direct-list-random', async (t) => {
        ...
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-random', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["count01", "output_field01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        ...
        const result = await client.direct({
            path: 'random/{count}/{output_field}',
            method: 'GET',
            params,
            query,
        });
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['POETRYDB_TEST_RANDOM_ENTID'];
