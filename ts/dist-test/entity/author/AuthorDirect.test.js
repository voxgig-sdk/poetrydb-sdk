...
(0, node_test_1.describe)('AuthorDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-author', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-author', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            params.id = "Ernest Dowson";
        }
        else {
            params.id = 'direct01';
        }
    ...
    (0, node_test_1.test)('direct-list-author', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-author', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["author01", "format01", "output_field01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            params.author = setup.idmap['author01'];
            params.format = setup.idmap['format01'];
            params.output_field = setup.idmap['output_field01'];
        }
        else {
            params.author = 'direct01';
            params.format = 'direct02';
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.PoetrydbSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['POETRYDB_TEST_AUTHOR_ENTID'];
    ...
    const client = new __1.PoetrydbSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=AuthorDirect.test.js.map