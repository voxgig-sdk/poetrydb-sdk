...
describe('LinecountDirect', async () => {
  ...
  test('direct-load-linecount', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-linecount', setup.live)) return
  ...
  test('direct-list-linecount', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-linecount', setup.live)) return
    if (skipIfMissingIds(t, setup, ["format01","linecount01","output_field01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['POETRYDB_TEST_LINECOUNT_ENTID']
