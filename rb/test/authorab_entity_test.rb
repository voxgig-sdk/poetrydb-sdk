...
class AuthorabEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Authorab(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Authorab(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = authorab_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "authorab." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set POETRYDB_TEST_AUTHORAB_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    authorab_ref01_data = nil
    if authorab_ref01_data_raw.length > 0
      authorab_ref01_data = Helpers.to_map(authorab_ref01_data_raw[0][1])
    end

    # LIST
    authorab_ref01_ent = client.Authorab(nil)
...
def authorab_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "authorab", "AuthorabTestData.json")
  ...
  entid_env_raw = ENV["POETRYDB_TEST_AUTHORAB_ENTID"]
