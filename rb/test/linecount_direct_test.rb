...
class LinecountDirectTest < Minitest::Test
  def test_direct_list_linecount
    setup = linecount_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-linecount", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["format01", "linecount01", "output_field01"].each do |_live_key|
  ...
  def test_direct_load_linecount
    setup = linecount_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-linecount", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def linecount_direct_setup(mockres)
