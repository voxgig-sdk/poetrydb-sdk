...
class LinecountEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Linecount(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Linecount(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = linecount_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "linecount." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set POETRYDB_TEST_LINECOUNT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    linecount_ref01_data = nil
    if linecount_ref01_data_raw.length > 0
      linecount_ref01_data = Helpers.to_map(linecount_ref01_data_raw[0][1])
    end

    # LIST
    linecount_ref01_ent = client.Linecount(nil)
...
def linecount_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "linecount", "LinecountTestData.json")
  ...
  entid_env_raw = ENV["POETRYDB_TEST_LINECOUNT_ENTID"]
