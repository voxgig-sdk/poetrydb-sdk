...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = PoetrydbOmni.make_runner(TEST_JSON_FILE, PoetrydbSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : PoetrydbOmni.errify(err)
    end
