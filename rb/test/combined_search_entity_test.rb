...
class CombinedSearchEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.CombinedSearch(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.CombinedSearch(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    combined_search_ref01_list_result = combined_search_ref01_ent.list(combined_search_ref01_match, nil)
