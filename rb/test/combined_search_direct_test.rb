...
class CombinedSearchDirectTest < Minitest::Test
  def test_direct_list_combined_search
    setup = combined_search_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-combined_search", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["input_field101", "input_field201", "search_term101", "search_term201"].each do |_live_key|
...
def combined_search_direct_setup(mockres)
