...
class AuthorDirectTest < Minitest::Test
  def test_direct_list_author
    setup = author_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-author", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["author01", "format01", "output_field01"].each do |_live_key|
