...
class RandomEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set POETRYDB_TEST_RANDOM_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def random_basic_setup(extra)
  ...
  idmap = Vs.transform(
    ["random01", "random02", "random03", "count01", "output_field01"],
    {
      "`$PACK`" => ["", {
        "`$KEY`" => "`$COPY`",
  ...
  entid_env_raw = ENV["POETRYDB_TEST_RANDOM_ENTID"]
