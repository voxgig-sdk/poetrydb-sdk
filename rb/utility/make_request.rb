...
module PoetrydbUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = PoetrydbResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = PoetrydbResponse.new(fetched)
    else
