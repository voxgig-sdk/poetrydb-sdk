...
class LinecountEntity
  ...
  def make
    opts = @_entopts.dup
    LinecountEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = PoetrydbHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Linecount, Hash] the current Linecount data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = PoetrydbHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Linecount fields)
  def match_get
  ...
  #
  # @param reqmatch [LinecountLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Linecount.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Linecount, Hash] the loaded Linecount; raises PoetrydbError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = PoetrydbHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [LinecountListMatch, Hash, nil] match filter (any subset of
  #   Linecount fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Linecount>, Array] the matching Linecount items; raises PoetrydbError on failure
