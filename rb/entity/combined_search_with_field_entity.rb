...
class CombinedSearchWithFieldEntity
  def initialize(client, entopts = nil)
    ...
    @_name = "combined_search_with_field"
    @_client = client
  ...
  def make
    opts = @_entopts.dup
    CombinedSearchWithFieldEntity.new(@_client, opts)
  end
  ...
  #
  # @param reqmatch [CombinedSearchWithFieldListMatch, Hash, nil] match filter (any subset of
  #   CombinedSearchWithField fields); defaults to nil, treated as an empty match that lists all.
