...
The Python SDK for the Poetrydb API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Author(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.Poetrydb(None).load(
    {"id": "test01"}, None
)
...
```
py/
├── poetrydb_sdk.py         -- Main SDK module
