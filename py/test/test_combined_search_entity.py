...
class TestCombinedSearchEntity:

    def test_should_create_instance(self):
        testsdk = PoetrydbSDK.test(None, None)
        ent = testsdk.CombinedSearch(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.CombinedSearch(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.CombinedSearch(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _combined_search_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "combined_search." + _op, "live" if _live else "unit")
            if _skip:
...
def _combined_search_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/combined_search/CombinedSearchTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("POETRYDB_TEST_COMBINED_SEARCH_ENTID"))
