...
class TestLinecountEntity:

    def test_should_create_instance(self):
        testsdk = PoetrydbSDK.test(None, None)
        ent = testsdk.Linecount(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Linecount(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Linecount(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _linecount_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "linecount." + _op, "live" if _live else "unit")
            if _skip:
        ...
        linecount_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(linecount_ref01_data_dt0_loaded))
        assert linecount_ref01_data_dt0_load_result is not None
        assert linecount_ref01_data_dt0_load_result["id"] == linecount_ref01_data["id"]



def _linecount_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/linecount/LinecountTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("POETRYDB_TEST_LINECOUNT_ENTID"))
