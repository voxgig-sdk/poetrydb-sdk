...
class TestCombinedSearchWithFieldDirect:

    def test_should_direct_list_combined_search_with_field(self):
        setup = _combined_search_with_field_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-combined_search_with_field", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["input_field101", "input_field201", "output_field01", "search_term101", "search_term201"]:
...
def _combined_search_with_field_direct_setup(mockres):
