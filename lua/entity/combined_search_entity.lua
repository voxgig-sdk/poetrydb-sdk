...
local CombinedSearchEntity = {}
CombinedSearchEntity.__index = CombinedSearchEntity


function CombinedSearchEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CombinedSearchEntity)
  self._name = "combined_search"
  self._client = client
...
function CombinedSearchEntity:get_name()
  return self._name
end


function CombinedSearchEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CombinedSearchEntity.new(self._client, opts)
end
...
function CombinedSearchEntity:mark_deleted()
  self._deleted = true
end


function CombinedSearchEntity:deleted()
  return true == self._deleted
end


function CombinedSearchEntity:data_set(args)
  if args ~= nil then
...
function CombinedSearchEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CombinedSearchEntity:match_set(args)
  if args ~= nil then
...
function CombinedSearchEntity:match_get()
...
function CombinedSearchEntity:stream(action, args, callopts)
...
function CombinedSearchEntity:_run_op(ctx, post_done)
  local utility = self._utility
