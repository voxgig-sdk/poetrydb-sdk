...
local LinecountEntity = {}
LinecountEntity.__index = LinecountEntity


function LinecountEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LinecountEntity)
  self._name = "linecount"
  self._client = client
...
function LinecountEntity:get_name()
  return self._name
end


function LinecountEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LinecountEntity.new(self._client, opts)
end
...
function LinecountEntity:mark_deleted()
  self._deleted = true
end


function LinecountEntity:deleted()
  return true == self._deleted
end


function LinecountEntity:data_set(args)
  if args ~= nil then
...
function LinecountEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LinecountEntity:match_set(args)
  if args ~= nil then
...
function LinecountEntity:match_get()
...
function LinecountEntity:stream(action, args, callopts)
...
function LinecountEntity:_run_op(ctx, post_done)
  local utility = self._utility
