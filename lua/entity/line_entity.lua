...
local LineEntity = {}
LineEntity.__index = LineEntity


function LineEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LineEntity)
  self._name = "line"
  self._client = client
...
function LineEntity:get_name()
  return self._name
end


function LineEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LineEntity.new(self._client, opts)
end
...
function LineEntity:mark_deleted()
  self._deleted = true
end


function LineEntity:deleted()
  return true == self._deleted
end


function LineEntity:data_set(args)
  if args ~= nil then
...
function LineEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LineEntity:match_set(args)
  if args ~= nil then
...
function LineEntity:stream(action, args, callopts)
...
function LineEntity:_run_op(ctx, post_done)
  local utility = self._utility
