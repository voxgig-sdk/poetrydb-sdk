...
local AuthorabEntity = {}
AuthorabEntity.__index = AuthorabEntity


function AuthorabEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AuthorabEntity)
  self._name = "authorab"
  self._client = client
...
function AuthorabEntity:get_name()
  return self._name
end


function AuthorabEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AuthorabEntity.new(self._client, opts)
end
...
function AuthorabEntity:mark_deleted()
  self._deleted = true
end


function AuthorabEntity:deleted()
  return true == self._deleted
end


function AuthorabEntity:data_set(args)
  if args ~= nil then
...
function AuthorabEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AuthorabEntity:match_set(args)
  if args ~= nil then
...
function AuthorabEntity:match_get()
...
function AuthorabEntity:stream(action, args, callopts)
...
---@param reqmatch AuthorabListMatch
---@param ctrl? table
---@return Authorab[]
---@return string? err
function AuthorabEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function AuthorabEntity:_run_op(ctx, post_done)
  local utility = self._utility
