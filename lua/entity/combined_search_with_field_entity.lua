-- Poetrydb SDK CombinedSearchWithField entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local CombinedSearchWithFieldEntity = {}
CombinedSearchWithFieldEntity.__index = CombinedSearchWithFieldEntity


function CombinedSearchWithFieldEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CombinedSearchWithFieldEntity)
  self._name = "combined_search_with_field"
  self._client = client
...
function CombinedSearchWithFieldEntity:get_name()
  return self._name
end


function CombinedSearchWithFieldEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CombinedSearchWithFieldEntity.new(self._client, opts)
end
...
function CombinedSearchWithFieldEntity:mark_deleted()
  self._deleted = true
end


function CombinedSearchWithFieldEntity:deleted()
  return true == self._deleted
end


function CombinedSearchWithFieldEntity:data_set(args)
  if args ~= nil then
...
function CombinedSearchWithFieldEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CombinedSearchWithFieldEntity:match_set(args)
  if args ~= nil then
...
function CombinedSearchWithFieldEntity:match_get()
...
function CombinedSearchWithFieldEntity:stream(action, args, callopts)
...
---@param reqmatch CombinedSearchWithFieldListMatch
---@param ctrl? table
---@return CombinedSearchWithField[]
---@return string? err
function CombinedSearchWithFieldEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CombinedSearchWithFieldEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return CombinedSearchWithFieldEntity
