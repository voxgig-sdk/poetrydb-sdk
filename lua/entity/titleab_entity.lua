-- Poetrydb SDK Titleab entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local TitleabEntity = {}
TitleabEntity.__index = TitleabEntity


function TitleabEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TitleabEntity)
  self._name = "titleab"
  self._client = client
...
function TitleabEntity:get_name()
  return self._name
end


function TitleabEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TitleabEntity.new(self._client, opts)
end
...
function TitleabEntity:mark_deleted()
  self._deleted = true
end


function TitleabEntity:deleted()
  return true == self._deleted
end


function TitleabEntity:data_set(args)
  if args ~= nil then
...
function TitleabEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TitleabEntity:match_set(args)
  if args ~= nil then
...
function TitleabEntity:match_get()
...
function TitleabEntity:stream(action, args, callopts)
...
---@param reqmatch TitleabListMatch
---@param ctrl? table
---@return Titleab[]
---@return string? err
function TitleabEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function TitleabEntity:_run_op(ctx, post_done)
  local utility = self._utility
