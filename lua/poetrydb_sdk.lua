...
-- by passing it.
function PoetrydbSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function PoetrydbSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "PoetrydbSDK: graphql: " .. msg
    res.graphql = errors
  end
