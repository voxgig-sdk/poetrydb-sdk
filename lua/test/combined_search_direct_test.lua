...
describe("CombinedSearchDirect", function()
  it("should direct-list-combined_search", function()
    local setup = combined_search_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-combined_search", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"input_field101", "input_field201", "search_term101", "search_term201"}) do
