...
describe("LinecountDirect", function()
  it("should direct-list-linecount", function()
    local setup = linecount_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-linecount", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"format01", "linecount01", "output_field01"}) do
  ...
  it("should direct-load-linecount", function()
    local setup = linecount_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-linecount", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function linecount_direct_setup(mockres)
