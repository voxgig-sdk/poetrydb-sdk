...
class FtCtrl extends PoetrydbControl
{
...
class FtClient
{
    ...
    private PoetrydbUtility $utility;

    public function __construct(PoetrydbUtility $utility, array $options)
    {
    ...
    public function get_utility(): PoetrydbUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PoetrydbContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public PoetrydbContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PoetrydbContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PoetrydbConfig::make_feature($name);
    ...
    private static function buildUrl(PoetrydbSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PoetrydbError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PoetrydbResponse($fetched);
        }
    ...
    private function fail(PoetrydbContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PoetrydbContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
